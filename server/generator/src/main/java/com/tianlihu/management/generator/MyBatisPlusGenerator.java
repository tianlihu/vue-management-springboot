package com.tianlihu.management.generator;
...
import com.tianlihu.management.config.GeneratorConfig;
...
public class MyBatisPlusGenerator implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        gc.setOutputDir(projectPath + "/dist/java");
...
//        // 关闭默认 xml 生成，调整生成 至 根目录
        ...
        // packageConfig.setEntity("com.tianlihu.model");
        // packageConfig.setMapper("com.tianlihu.mapper");
        // packageConfig.setController("com.tianlihu.controller");
        // packageConfig.setService("com.tianlihu.service");
        // packageConfig.setServiceImpl("com.tianlihu.service.impl");
        ...
        focList.add(new FileOutConfig("/templates/mapper.xml.ftl") {
            ...
            public String outputFile(TableInfo tableInfo) {
                return projectPath + "/dist/mapping/" + tableInfo.getEntityName() + "Mapper.xml";
            }
        });
...
//            @Override
//            public String outputFile(TableInfo tableInfo) {
//                return projectPath + "/dist/consumer/" + config.getBasepackage().replace('.', '/') + "/api/" + tableInfo.getEntityName() + "Api.java";
//            }
//        });
//        focList.add(new FileOutConfig("/ftl/consumer.java.ftl") {
//            @Override
//            public String outputFile(TableInfo tableInfo) {
//                return projectPath + "/dist/consumer/" + config.getBasepackage().replace('.', '/') + "/controller/" + tableInfo.getEntityName() + "Controller.java";
//            }
//        });
        focList.add(new FileOutConfig("/ftl/query.java.ftl") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                return projectPath + "/dist/java/" + config.getBasepackage().replace('.', '/') + "/query/" + tableInfo.getEntityName() + "Query.java";
            }
        });
        focList.add(new FileOutConfig("/ftl/entity.java.ftl") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                return projectPath + "/dist/java/" + config.getBasepackage().replace('.', '/') + "/entity/" + tableInfo.getEntityName() + ".java";
            }
        });
...
//        });
        ...
        // strategy.setSuperServiceClass("com.tianlihu.util.MyService");
        // strategy.setSuperServiceImplClass("com.tianlihu.util.MyServiceImpl");
        // 实体类是否带有lombok风格
